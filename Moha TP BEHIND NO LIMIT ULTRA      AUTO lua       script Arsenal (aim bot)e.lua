...
--   3. Aimbot + Auto-Kill : vise automatiquement l'ennemi visible le plus proche, et tire jusqu'à ce qu'il meure.
--      (Activation/désactivation avec la touche J)
--   4. TP Behind : se téléporte derrière un ennemi et le suit jusqu'à sa mort.
--      (Activation/désactivation avec la touche K)
--   5. Speed Modifier : touches X/Z pour boost/reset, C/V pour ajuster la vitesse
--   6. Infinite Bullet : réinitialise en continu l'ammo de tes outils
...
--                        AIMBOT + AUTO-KILL
------------------------------------------------------------
local aimbotEnabled = true  -- Toggle du système aimbot/auto-kill (J)
local currentTarget = nil
local raycastParams = RaycastParams.new()
local holdingMouse = false
local VirtualInputManager = game:GetService("VirtualInputManager")

-- Configurer Raycast pour ignorer ton personnage
...
local function toggleAimbot()
    ...
    if not aimbotEnabled then
        ...
        sendNotification("Script", "[AIMBOT] Désactivé", 5, image)
    else
        print("[AIMBOT] Activé")
        sendNotification("Script", "[AIMBOT] Activé", 5, image)
    end
end
...
-- Boucle principale aimbot + auto-kill
RunService.RenderStepped:Connect(function()
    if not aimbotEnabled then
        stopShooting()
        return
...
------------------------------------------------------------
--                TP BEHIND ENNEMI (TP Behind)
------------------------------------------------------------
-- Ce système se téléporte derrière un ennemi en continu tant qu'il est vivant,
-- sans vérifier sa visibilité. La touche K active/désactive ce mode.

local tpBehindEnabled = false
local tpBehindCurrentTarget = nil
local tpBehindOffset = 5  -- Distance en studs derrière l'ennemi

-- Fonction pour obtenir le joueur ennemi le plus proche sans vérification de visibilité
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer 
           ...
           and player.Character:FindFirstChild("Head") then
            -- Ignorer les joueurs de la même équipe
            if LocalPlayer.Team and player.Team and player.Team == LocalPlayer.Team then
                continue
            end
            local humanoid = player.Character:FindFirstChildOfClass("Humanoid")
...
-- Toggle TP Behind avec la touche K
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode.K then
        tpBehindEnabled = not tpBehindEnabled
        if not tpBehindEnabled then
            tpBehindCurrentTarget = nil
            sendNotification("Script", "TP Behind désactivé", 5, image)
        else
            sendNotification("Script", "TP Behind activé", 5, image)
        end
    end
end)

-- Boucle principale TP Behind
RunService.RenderStepped:Connect(function()
    if tpBehindEnabled then
        local target = nil
        -- Vérifie si la cible actuelle est toujours valide
        if tpBehindCurrentTarget then
            if not tpBehindCurrentTarget.Character 
               or not tpBehindCurrentTarget.Character:FindFirstChild("HumanoidRootPart") then
                tpBehindCurrentTarget = nil
            else
                ...
                if not targetHumanoid or targetHumanoid.Health <= 0 
                   ...
                   or targetHRP.Position.Y < -50 then
                    tpBehindCurrentTarget = nil
                else
        ...
        -- Si aucune cible valide, cherche la plus proche (sans vérifier la visibilité)
        if not target then
            target = getClosestEnemy()
            tpBehindCurrentTarget = target
        end

        -- Téléporte derrière la cible si elle est trouvée
...
------------------------------------------------------------
--   TELEPORT SI EN DESSOUS DE LA MAP VERS LE JOUEUR LE PLUS ÉLOIGNÉ
------------------------------------------------------------

-- Fonction qui trouve le joueur le plus éloigné de toi, en excluant ceux de ta team
...
-- Boucle qui vérifie si tu es en dessous de la map et te téléporte vers le joueur le plus éloigné (qui n'est pas dans ta team)
RunService.RenderStepped:Connect(function()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        local hrp = LocalPlayer.Character.HumanoidRootPart
        -- Si ta position Y est inférieure à -50, tu es considéré comme "en dessous de la map"
        if hrp.Position.Y < -50 then
            local farthestPlayer = getFarthestPlayer()
            if farthestPlayer 
               and farthestPlayer.Character 
               and farthestPlayer.Character:FindFirstChild("HumanoidRootPart") then
                local farthestHRP = farthestPlayer.Character.HumanoidRootPart
                -- Téléporte ton personnage vers la position du joueur le plus éloigné, avec un petit offset vertical
...
sendNotification(
    "Script",
    "God Mode, ESP, TP, Aimbot + Auto-Kill, Speed Modifier, Infinite Bullet & TP Behind activés.\nRMB pour Aimbot,\nX/Z pour boost/reset speed,\nC/V pour ajuster la vitesse.\nJ pour activer/désactiver l'Aimbot + Auto-Kill.\nK pour activer/désactiver le TP Behind.",
    15,
    image
)
