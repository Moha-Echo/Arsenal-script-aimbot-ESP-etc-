...
-- Dès qu'il est détecté comme mort ou hors map, il recherche automatiquement un nouvel ennemi.
-- La touche K active/désactive ce mode.
    ...
    if tpBehindEnabled then
        ...
        if tpBehindCurrentTarget then
            if not tpBehindCurrentTarget.Character or not tpBehindCurrentTarget.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- Vérifie si le joueur est mort, si son état est "Dead" ou s'il est hors map (position Y trop basse)
                if not targetHumanoid or targetHumanoid.Health <= 0 or targetHumanoid:GetState() == Enum.HumanoidStateType.Dead or targetHRP.Position.Y < -50 then
                    tpBehindCurrentTarget = nil
                else
